...
def write_results_score(filename, results):
    save_format = '{frame},{id},{x1},{y1},{w},{h},{cls},{s},-1,-1,-1\n'
    with open(filename, 'w') as f:
        for i in range(results.shape[0]):
            ...
            cls = frame_data[6]
            line = save_format.format(frame=frame_id, id=track_id, x1=x1, y1=y1, w=w, h=h, cls=cls, s=-1)
            f.write(line)
...
def dti(txt_path, save_path, n_min=25, n_dti=20):
    ...
    for seq_txt in seq_txts:
        ...
        seq_results = np.zeros((1, 11), dtype=np.float64)
        for track_id in range(min_id, max_id + 1):
            ...
            if n_frame > n_min:
                ...
                if num_dti > 0:
                    data_dti = np.zeros((num_dti, 11), dtype=np.float64)
                    for n in range(num_dti):
                        ...
                        data_dti[n, 6] = tracklet[i, 6]
                        data_dti[n, 7:] = [1, -1, -1, -1]
